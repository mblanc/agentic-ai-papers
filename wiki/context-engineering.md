...
{{gh:upstash/context7}}, Mirage mounts S3/Slack/Gmail/GitHub/Redis as one
virtual filesystem so the agent can use bash {{gh:strukto-ai/mirage}}, and
OpenViking unifies memory, resources and skills behind the same paradigm
{{gh:volcengine/openviking}}. Token-reduction tools attack the same problem from
the output side: LLMLingua compresses prompts up to 20x
{{gh:microsoft/llmlingua}}, headroom cuts tool/log/RAG output 60-95% before it
hits context {{gh:chopratejas/headroom}}, and dirac uses hash-anchored edits and
AST manipulation for 50-80% cost reduction {{gh:dirac-run/dirac}}.
...
{{arxiv:2602.05447}}. Structure isn't free, and "give the agent files" is not a
...
{{arxiv:2601.21557}}, and CEDAR applies structured, interleaved plan/code
context specifically to agentic data science {{arxiv:2601.06606}}.
...
incidental plumbing {{url:https://openai.com/index/harness-engineering}}. Claude
Code's compaction work is the applied instance most people encounter first
{{url:https://platform.claude.com/docs/en/build-with-claude/compaction}}, and is
covered on its own page: [[context-compaction]].

## Open problems

The filesystem/file-native pattern's benefit is model-dependent, not universal
{{arxiv:2602.05447}} — there is no settled guidance yet on when to reach for it.
